...
$__newAttributes = [];
$__propNames = \Illuminate\View\ComponentAttributeBag::extractPropNames((['position' => 'top-center']));
...
foreach (array_filter((['position' => 'top-center']), 'is_string', ARRAY_FILTER_USE_KEY) as $__key => $__value) {
    $$__key = $$__key ?? $__value;
}
...
<div x-data="{ 
        ...
        $nextTick(() => {
            ...
            <?php if(session('success')): ?> add('success', <?php echo \Illuminate\Support\Js::from(session('success')); ?>); <?php endif; ?>
            <?php if(session('error')): ?> add('error', <?php echo \Illuminate\Support\Js::from(session('error')); ?>); <?php endif; ?>
            <?php if(session('warning')): ?> add('warning', <?php echo \Illuminate\Support\Js::from(session('warning')); ?>); <?php endif; ?>
            <?php if(session('info')): ?> add('info', <?php echo \Illuminate\Support\Js::from(session('info')); ?>); <?php endif; ?>
            <?php if($errors->any()): ?> add('error', <?php echo \Illuminate\Support\Js::from($errors->first()); ?>); <?php endif; ?>
        });
        
        window.addEventListener('notify', event => {
            add(event.detail.type || 'info', event.detail.message);
        });
    " class="fixed <?php echo e($classes); ?> z-[10000] flex flex-col gap-2 w-full max-w-sm pointer-events-none">
...
<?php /**PATH /home/ubuntu/metis/resources/views/components/ui/toaster.blade.php ENDPATH**/ ?>