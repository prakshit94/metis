            ...
            @foreach($customer->orders as $order)
                <div class="card overflow-hidden transition-all">
                    ...
                    <div @click="expandedOrder = expandedOrder === {{ $order->id }} ? null : {{ $order->id }}" class="card-body p-4 cursor-pointer hover-bg-body-tertiary d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-4">
                        <div class="d-flex align-items-center gap-4">
                            ...
                            <div>
                                <div class="d-flex align-items-center gap-2 mb-1">
                                    <h3 class="h5 fw-bold mb-0 text-body-emphasis">{{ $order->order_no }}</h3>
                                    @php
                        ...
                        <div class="d-flex align-items-center gap-4">
                            <div class="text-end">
                                ...
                                <p class="mb-0 fw-black text-body-emphasis fs-5">₹ {{ number_format($order->net_amount, 2) }}</p>
                            </div>
                            <div class="bg-body-tertiary text-secondary rounded-circle d-flex align-items-center justify-content-center transition-all" style="width: 32px; height: 32px;" :class="expandedOrder === {{ $order->id }} ? 'bg-secondary text-white' : ''">
                    ...
                    <div x-show="expandedOrder === {{ $order->id }}" x-collapse x-cloak>
                        <div class="card-body p-4 p-md-5 border-top bg-body-tertiary bg-opacity-50 d-flex flex-column gap-4">
                            ...
                            <div class="d-flex flex-wrap align-items-center gap-2">
                                ...
                                <a href="{{ route('orders.receipt', $order->id) }}" class="btn btn-sm btn-outline-secondary border fw-bold text-uppercase d-flex align-items-center gap-2 shadow-sm rounded-pill px-3 text-body-emphasis" style="font-size: 10px; letter-spacing: 1px;">
                            ...
                            {{-- Items Table --}}
                            <div class="card overflow-hidden mt-3 mb-4">
<div class="card-header bg-body border-bottom py-3 d-flex justify-content-between align-items-center">
                                        ...
                                        <tbody class="border-top-0">
                                            @if(isset($order->items))
                                                @foreach($order->items as $item)
                                                    <tr>
                                                        <td class="px-4 py-3">
                                                            <p class="mb-0 fw-bold text-body-emphasis">{{ $item->product?->name ?? 'Unknown Product' }}</p>
                                                        ...
                                                        <td class="px-4 py-3 text-center">
                                                            <span class="badge bg-body-tertiary text-body-emphasis border">{{ $item->quantity ?? 1 }}</span>
                                                        </td>
                                                        <td class="px-4 py-3 text-end fw-bold text-body-emphasis fs-6">
        ...
        @else
            <div class="text-center py-5 px-4 rounded-4 border border-2 border-dashed bg-body-tertiary">
                ...
                <h4 class="h5 fw-bold text-body-emphasis">No Order History</h4>
...
.hover-bg-body-tertiary:hover { background-color: rgba(var(--bs-light-rgb), 0.5) !important; }
</style>
