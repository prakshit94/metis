                    ...
                    <div class="card h-100 border-0 shadow-sm rounded-4 position-relative hover-shadow-lg transition-all">
                        ...
                        <div class="card-body p-4 d-flex flex-column">
                            <div class="d-flex align-items-center gap-3 mb-4">
                                ...
                                <span class="fw-bold text-uppercase text-dark" style="font-size: 12px; letter-spacing: 1px;">{{ $address->label ?: 'Address' }}</span>
                            </div>

                            <p class="fw-bold text-dark mb-1 fs-6">{{ $address->address_line_1 }}</p>
                            ...
                            <div class="mt-auto pt-3 border-top d-flex flex-column gap-2">
                                <div class="d-flex justify-content-between">
                                    ...
                                    <span class="fw-bold text-dark" style="font-size: 12px;">{{ $address->village?->village_name ?? $address->village_name ?? '—' }}</span>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <span class="text-muted fw-bold text-uppercase" style="font-size: 9px; letter-spacing: 1px;">Post Office</span>
                                    <span class="fw-bold text-dark" style="font-size: 12px;">{{ $address->village?->post_so_name ?? $address->post_office ?? '—' }}</span>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <span class="text-muted fw-bold text-uppercase" style="font-size: 9px; letter-spacing: 1px;">Taluka</span>
                                    <span class="fw-bold text-dark" style="font-size: 12px;">{{ $address->village?->taluka_name ?? $address->taluka ?? '—' }}</span>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <span class="text-muted fw-bold text-uppercase" style="font-size: 9px; letter-spacing: 1px;">District</span>
                                    <span class="fw-bold text-dark" style="font-size: 12px;">{{ $address->village?->district_name ?? $address->city ?? '—' }}</span>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <span class="text-muted fw-bold text-uppercase" style="font-size: 9px; letter-spacing: 1px;">State</span>
                                    <span class="fw-bold text-dark" style="font-size: 12px;">{{ !empty($address->village?->state_name) ? $address->village->state_name : (!empty($address->state) ? $address->state : '—') }}</span>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <span class="text-muted fw-bold text-uppercase" style="font-size: 9px; letter-spacing: 1px;">Pincode</span>
                                    <span class="fw-bold text-dark font-monospace" style="font-size: 12px;">{{ $address->village?->pincode ?? $address->pincode ?? '—' }}</span>
                                </div>
                        ...
                        <div class="card-footer bg-transparent border-top p-3 d-flex justify-content-end gap-2">
                            <button @click="openEditModal({{ $address->toJson() }})" class="btn btn-sm btn-light text-primary border rounded-3 px-3 shadow-sm">
                                <i class="bi bi-pencil-square"></i>
                            </button>
                            <button @click="openDeleteModal({{ $address->toJson() }})" class="btn btn-sm btn-light text-danger border rounded-3 px-3 shadow-sm">
