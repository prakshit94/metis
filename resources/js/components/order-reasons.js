  ...
  Alpine.data('orderReasonsTable', () => ({
    ...
    get tabTitle() {
      switch (this.activeTab) {
        case 'reschedule':
          return 'Reschedule Reasons Directory';
        case 'return':
          return 'Return Reasons Directory';
        case 'failure':
          return 'Delivery Failure Reasons Directory';
        case 'cancel':
          return 'Cancellation Reasons Directory';
        default:
          return 'Directory';
      }
    },
    ...
    get filteredReasons() {
      ...
      if (this.searchQuery) {
        ...
        result = result.filter(
          (r) => r.reason.toLowerCase().includes(q) || String(r.id).includes(q)
        );
      }
    ...
    async bulkAction(action) {
      ...
      const actionText =
        action === 'delete' ? 'delete' : action === 'activate' ? 'activate' : 'deactivate';
      if (
        !confirm(
          `Are you sure you want to ${actionText} ${this.selectedReasons.length} selected reasons?`
        )
      ) {
        return;
      }
        ...
        for (const id of this.selectedReasons) {
          try {
            ...
            if (action === 'delete') {
              ...
              if (
                reason &&
                ((action === 'activate' && !reason.is_active) ||
                  (action === 'deactivate' && reason.is_active))
              ) {
