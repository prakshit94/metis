      ...
      if (checked) {
        ...
        async saveItem() {
            ...
            try {
                ...
                showToast(`Successfully ${this.isEditing ? 'updated' : 'created'} unit of measure.`, 'success');
            ...
            try {
                if (action === 'delete') {
                    const confirmed = await confirmDelete({
                        ...
                        text: `Are you sure you want to delete ${this.selectedItems.length} units?`
                    });
                    if (!confirmed) return;
        ...
        exportData() {
            ...
            a.setAttribute('download', 'uom_export.csv');
            a.click();
            URL.revokeObjectURL(url);
        }
    };

    window.Alpine.store('uomTable', instance);
    return instance;
};
