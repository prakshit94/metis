    ...
    const instance = {
        ...
        async saveItem() {
            ...
            try {
                ...
                showToast(`Successfully ${this.isEditing ? 'updated' : 'created'} brand.`, 'success');
            ...
            try {
                if (action === 'delete') {
                    const confirmed = await confirmDelete({
                        ...
                        text: `Are you sure you want to delete ${this.selectedItems.length} brands?`
                    });
                    if (!confirmed) return;
        ...
        exportData() {
            ...
            a.setAttribute('download', 'brands_export.csv');
            a.click();
            URL.revokeObjectURL(url);
        }
    };

    window.Alpine.store('brandsTable', instance);
    return instance;
};
