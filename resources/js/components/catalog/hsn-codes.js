    ...
    const instance = {
      ...
      if (checked) {
        ...
        editItem(item) {
            this.isEditing = true;
            this.form = { ...item };
            // HSN uses code as the primary identifier; map it to name for the form field
            this.form.name = item.code || item.name || '';
            this.modalInstance?.show();
        },
        ...
        async saveItem() {
            ...
            try {
                ...
                // HSN codes store the code field as primary
                ...
                showToast(`Successfully ${this.isEditing ? 'updated' : 'created'} HSN code.`, 'success');
        ...
        async deleteItem(item) {
            ...
            try {
                ...
                showToast('HSN code deleted successfully.', 'success');
                await this.loadData();
            } catch (error) {
                showToast(error.message, 'error');
            }
            ...
            try {
                if (action === 'delete') {
                    const confirmed = await confirmDelete({
                        ...
                        text: `Are you sure you want to delete ${this.selectedItems.length} HSN codes?`
                    });
                    if (!confirmed) return;
        ...
        exportData() {
            ...
            const headers = ['ID', 'HSN Code', 'Description', 'Tax Rate (%)', 'Status', 'Created At'];
            ...
            a.setAttribute('download', 'hsn_codes_export.csv');
            a.click();
            URL.revokeObjectURL(url);
        }
    };

    window.Alpine.store('hsnCodesTable', instance);
    return instance;
};
