...
export default () => {
  const instance = {
    ...
    get filteredProviderUsers() {
      ...
      return this.providerUsers.filter(
        (u) =>
    ...
    validatePriorities(selectedIdsArray, prioritiesObj) {
      ...
      for (const id of selectedIdsArray) {
        ...
        if (seen.has(pri)) {
          showToast(
            'Duplicate priority detected. Please ensure all priorities are unique.',
            'warning'
          );
      ...
      try {
        ...
        this.statusStats = [
          ...
          },
          {
            ...
            percentage: this.stats.total
              ? Math.round((this.stats.inactive / this.stats.total) * 100)
              : 0,
            color: '#64748b',
          },
    ...
    async bulkAssignProvider() {
      ...
      try {
        ...
        showToast(
          `Providers assigned successfully to ${this.selectedItems.length} service(s).`,
          'success'
        );
        this.selectedItems = [];
    ...
    async saveItem() {
      if (!this.validatePriorities(this.form.provider_user_ids, this.form.provider_priorities))
        return;
      ...
      try {
        ...
        showToast(
          `Successfully ${this.isEditing ? 'updated' : 'created'} shipping service.`,
          'success'
        );
