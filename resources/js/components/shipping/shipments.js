...
export default () => {
  const instance = {
    ...
    calculateProviders() {
      ...
      this.topProviders = Object.values(providerMap)
        .map((p, idx) => {
          ...
          const exceptionRate =
            activeTotal > 0 ? (((p.failed + p.returned) / activeTotal) * 100).toFixed(1) : '0.0';
    ...
    async loadData() {
      ...
      try {
        ...
        this.statusStats = [
          ...
          {
            name: 'In Transit',
            count: this.stats.in_transit,
            percentage: this.stats.total
              ? Math.round((this.stats.in_transit / this.stats.total) * 100)
              : 0,
    ...
    onStatusChange() {
      ...
      if (newStatus === 'delivered' || newStatus === 'returned') {
        ...
        if (
          ...
          this.selectedShipment.order.items
        ) {
