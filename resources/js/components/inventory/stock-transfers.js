...
export default () => ({
  ...
  form: {
    ...
    from_warehouse_id: '',
    to_warehouse_id: '',
    items: [{ product_id: '', quantity: 1, batch_number: '' }],
  },
  warehouseStocks: {},
  ...
  async fetchWarehouseStocks() {
    ...
    try {
      const data = await this.apiRequest(
        `/api/inventory/stocks?warehouse_id=${fromWarehouseId}&per_page=1000`
      );
      const stocks = {};
      if (data.data) {
        data.data.forEach((stock) => {
          stocks[stock.product_id] = Math.max(
            0,
            parseFloat(stock.quantity) - parseFloat(stock.reserved_qty)
          );
        });
      }
  ...
  async editItem(item) {
    ...
    if (this.form.items.length === 0)
      this.form.items = [{ product_id: '', quantity: 1, batch_number: '' }];
    await this.fetchWarehouseStocks();
    this.modalInstance?.show();
  },

  addItem() {
    this.form.items.push({ product_id: '', quantity: 1, batch_number: '' });
  },
  removeItem(index) {
  ...
  async saveItem() {
    ...
    const invalid = this.form.items.some(
      (i) => !i.product_id || !i.quantity || parseFloat(i.quantity) <= 0
    );
    if (invalid) {
    ...
    try {
      ...
      if (this.form.id) {
        await this.apiRequest(`/api/inventory/transfers/${this.form.id}`, {
          method: 'PUT',
          body: JSON.stringify(payload),
        });
        showToast('Transfer updated successfully.', 'success');
      } else {
        await this.apiRequest('/api/inventory/transfers', {
          method: 'POST',
          body: JSON.stringify(payload),
        });
  ...
  async sendTransfer(item) {
    const ok = await confirmAction({
      title: 'Mark as Sent?',
      text: `Mark transfer ${item.transfer_no} as dispatched?`,
  ...
  async receiveTransfer(item) {
    const ok = await confirmAction({
      ...
      text: `This will move stock from ${item.from_warehouse?.name} to ${item.to_warehouse?.name}.`,
      confirmText: 'Receive & Update Stock',
      icon: 'info',
    });
    if (!ok) return;
    try {
  ...
  async cancelTransfer(item) {
    const ok = await confirmAction({
      ...
      text: `Cancel transfer ${item.transfer_no}?`,
