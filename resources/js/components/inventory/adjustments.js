...
export default () => ({
  ...
  async saveItem() {
    ...
    const invalid = this.form.items.some(
      (i) =>
        !i.product_id || i.new_qty === undefined || i.new_qty === null || parseFloat(i.new_qty) < 0
    );
    if (invalid) {
      showToast(
        'Please select a product and valid non-negative new quantity for all items.',
        'error'
      );
      return;
    }
    ...
    try {
      ...
      if (this.form.id) {
        await this.apiRequest(`/api/inventory/adjustments/${this.form.id}`, {
          method: 'PUT',
          body: JSON.stringify(payload),
        });
        showToast('Adjustment updated successfully.', 'success');
      } else {
        await this.apiRequest('/api/inventory/adjustments', {
          method: 'POST',
          body: JSON.stringify(payload),
        });
  ...
  async approveItem(item) {
    const ok = await confirmAction({
      ...
      text: `This will update actual stock levels for ${item.items_count} product(s).`,
  ...
  async rejectItem(item) {
    const ok = await confirmAction({
      ...
      text: `Reject adjustment ${item.reference_no}? This cannot be undone.`,
